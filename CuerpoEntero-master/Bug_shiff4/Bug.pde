...
class Bug {
  ...
  void update(PVector n, PVector m) {
    ...
    if (position.x > width+50 || position.x < -50 || position.y > height+50 || position.y < -50) {
      ...
      acceleration.set(random(-5, 5), random(-5, 5));
    }
    ...
    position.x= constrain(position.x, -100, width+100);
    position.y= constrain(position.y, -100, height+100);
  }
